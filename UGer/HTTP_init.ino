...
// Установить код геркона по запросу вида  http://192.168.0.101/gerkon?gerkon=2866731
void handle_Set_gerkon() 
{              //

  gerkonopen = HTTP.arg("gerkonopen");         // Получаем значение ssidAP из запроса сохраняем в глобальной переменной
  gerkonclose = HTTP.arg("gerkonclose");         // Получаем значение ssidAP из запроса сохраняем в глобальной переменной
  gerkonbat = HTTP.arg("gerkonbat");         // Получаем значение ssidAP из запроса сохраняем в глобальной переменной
  gerkonalarm = HTTP.arg("gerkonalarm");         // Получаем значение ssidAP из запроса сохраняем в глобальной переменной
  saveConfigSetup();                         // Функция сохранения данных во Flash
  HTTP.send(200, "text/plain", "OK");   // отправляем ответ о выполнении

  Serial.print("gerkonopen - "); Serial.println(gerkonopen); 
  Serial.print("gerkonclose - "); Serial.println(gerkonclose); 
  Serial.print("gerkonbat - "); Serial.println(gerkonbat); 
  Serial.print("gerkonalarm - "); Serial.println(gerkonalarm);   
}
